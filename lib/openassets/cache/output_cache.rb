module OpenAssets
  module Cache
    ...
    class OutputCache < CacheBase
      ...
      def get(txid, index)
        rows = @db_provider.execute('SELECT Value,Script,AssetId,AssetQuantity,OutputType,Metadata FROM Output WHERE TransactionHash = ? AND OutputIndex = ?', [txid, index])
      ...
      def put(txid, index, output)
        @db_provider.execute("#{@db_provider.get_sql_insert_ignore()} INTO Output (TransactionHash, OutputIndex, Value,Script,AssetId,AssetQuantity,OutputType,Metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
